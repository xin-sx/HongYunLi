package com.hongyunli.calendar

import java.util.GregorianCalendar

/**
 * 农历计算工具类
 * 包含农历日期转换、天干地支、生肖等计算
 */
object LunarCalendar {

    // 天干
    private val tianGan = arrayOf("甲", "乙", "丙", "丁", "戊", "己", "庚", "辛", "壬", "癸")
    // 地支
    private val diZhi = arrayOf("子", "丑", "寅", "卯", "辰", "巳", "午", "未", "申", "酉", "戌", "亥")
    // 生肖
    private val zodiacArr = arrayOf("鼠", "牛", "虎", "兔", "龙", "蛇", "马", "羊", "猴", "鸡", "狗", "猪")
    // 农历月名
    private val lunarMonthNames = arrayOf("正", "二", "三", "四", "五", "六", "七", "八", "九", "十", "冬", "腊")
    // 农历日名
    private val lunarDayNames = arrayOf(
        "初一", "初二", "初三", "初四", "初五", "初六", "初七", "初八", "初九", "初十",
        "十一", "十二", "十三", "十四", "十五", "十六", "十七", "十八", "十九", "二十",
        "廿一", "廿二", "廿三", "廿四", "廿五", "廿六", "廿七", "廿八", "廿九", "三十"
    )

    /**
     * 农历数据表 1900-2100
     * 每个元素编码了对应年份的农历信息
     */
    private val lunarInfo = intArrayOf(
        0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950, 0x16554, 0x056a0, 0x09ad0, 0x055d2,
        0x04ae0, 0x0a5b6, 0x0a4d0, 0x0d250, 0x1d255, 0x0b540, 0x0d6a0, 0x0ada2, 0x095b0, 0x14977,
        0x04970, 0x0a4b0, 0x0b4b5, 0x06a50, 0x06d40, 0x1ab54, 0x02b60, 0x09570, 0x052f2, 0x04970,
        0x06566, 0x0d4a0, 0x0ea50, 0x06e95, 0x05ad0, 0x02b60, 0x186e3, 0x092e0, 0x1c8d7, 0x0c950,
        0x0d4a0, 0x1d8a6, 0x0b550, 0x056a0, 0x1a5b4, 0x025d0, 0x092d0, 0x0d2b2, 0x0a950, 0x0b557,
        0x06ca0, 0x0b550, 0x15355, 0x04da0, 0x0a5b0, 0x14573, 0x052b0, 0x0a9a8, 0x0e950, 0x06aa0,
        0x0aea6, 0x0ab50, 0x04b60, 0x0aae4, 0x0a570, 0x05260, 0x0f263, 0x0d950, 0x05b57, 0x056a0,
        0x096d0, 0x04dd5, 0x04ad0, 0x0a4d0, 0x0d4d4, 0x0d250, 0x0d558, 0x0b540, 0x0b6a0, 0x195a6,
        0x095b0, 0x049b0, 0x0a974, 0x0a4b0, 0x0b27a, 0x06a50, 0x06d40, 0x0af46, 0x0ab60, 0x09570,
        0x04af5, 0x04970, 0x064b0, 0x074a3, 0x0ea50, 0x06b58, 0x05ac0, 0x0ab60, 0x096d5, 0x092e0,
        0x0c960, 0x0d954, 0x0d4a0, 0x0da50, 0x07552, 0x056a0, 0x0abb7, 0x025d0, 0x092d0, 0x0cab5,
        0x0a950, 0x0b4a0, 0x0baa4, 0x0ad50, 0x055d9, 0x04ba0, 0x0a5b0, 0x15176, 0x052b0, 0x0a930,
        0x07954, 0x06aa0, 0x0ad50, 0x05b52, 0x04b60, 0x0a6e6, 0x0a4e0, 0x0d260, 0x0ea65, 0x0d530,
        0x05aa0, 0x076a3, 0x096d0, 0x04afb, 0x04ad0, 0x0a4d0, 0x1d0b6, 0x0d250, 0x0d520, 0x0dd45,
        0x0b5a0, 0x056d0, 0x055b2, 0x049b0, 0x0a577, 0x0a4b0, 0x0aa50, 0x1b255, 0x06d20, 0x0ada0,
        0x14b63, 0x09370, 0x049f8, 0x04970, 0x064b0, 0x168a6, 0x0ea50, 0x06b20, 0x1a6c4, 0x0aae0,
        0x092e0, 0x0d2e3, 0x0c960, 0x0d557, 0x0d4a0, 0x0da50, 0x05d55, 0x056a0, 0x0a6d0, 0x055d4,
        0x052d0, 0x0a9b8, 0x0a950, 0x0b4a0, 0x0b6a6, 0x0ad50, 0x055a0, 0x0aba4, 0x0a5b0, 0x052b0,
        0x0b273, 0x06930, 0x07337, 0x06aa0, 0x0ad50, 0x14b55, 0x04b60, 0x0a570, 0x054e4, 0x0d160,
        0x0e968, 0x0d520, 0x0daa0, 0x16aa6, 0x056d0, 0x04ae0, 0x0a9d4, 0x0a4d0, 0x0d150, 0x0f252,
        0x0d520
    )

    class LunarDate(
        val lunarYear: Int,
        val lunarMonth: Int,
        val lunarDay: Int,
        val isLeapMonth: Boolean,
        val ganZhiYear: String,
        val lunarMonthName: String,
        val lunarDayName: String,
        val zodiac: String
    )

    // 返回农历1900年1月1日到指定公历日期的天数差
    private fun daysBetweenSolarAndLunarBase(year: Int, month: Int, day: Int): Int {
        val baseDate = GregorianCalendar(1900, 0, 31) // 1900年1月31日 = 农历1900年正月初一
        val targetDate = GregorianCalendar(year, month - 1, day)
        val diff = ((targetDate.timeInMillis - baseDate.timeInMillis) / (1000 * 60 * 60 * 24)).toInt()
        return diff
    }

    // 获取农历某年的总天数
    private fun lunarYearDays(year: Int): Int {
        var sum = 348
        var info = lunarInfo[year - 1900]
        for (i in 0..11) {
            if ((info and 0x10000) != 0) sum += 1
            info = info shl 1
        }
        return sum + leapDays(year)
    }

    // 获取农历某年闰月的天数
    private fun leapDays(year: Int): Int {
        if (leapMonth(year) == 0) return 0
        return if ((lunarInfo[year - 1900] and 0x10000) != 0) 30 else 29
    }

    // 获取农历某年闰哪个月
    private fun leapMonth(year: Int): Int {
        return lunarInfo[year - 1900] and 0xf
    }

    // 获取农历某年某月的天数
    private fun lunarMonthDays(year: Int, month: Int): Int {
        return if ((lunarInfo[year - 1900] and (0x10000 shr month)) != 0) 30 else 29
    }

    fun solarToLunar(year: Int, month: Int, day: Int): LunarDate {
        val offset = daysBetweenSolarAndLunarBase(year, month, day)

        // 计算农历年
        var lunarYear = 1900
        var daysInYear = lunarYearDays(lunarYear)
        var temp = offset
        while (lunarYear < 2101 && temp >= daysInYear) {
            temp -= daysInYear
            lunarYear++
            daysInYear = lunarYearDays(lunarYear)
        }

        // 计算农历月
        val leap = leapMonth(lunarYear)
        var isLeap = false
        var lunarMonth = 1

        for (i in 1..13) {
            if (leap > 0 && i == leap + 1 && !isLeap) {
                // 闰月
                val monthDays = leapDays(lunarYear)
                if (temp < monthDays) {
                    return LunarDate(
                        lunarYear = lunarYear,
                        lunarMonth = leap,
                        lunarDay = temp + 1,
                        isLeapMonth = true,
                        ganZhiYear = getGanZhiYear(lunarYear),
                        lunarMonthName = "闰" + lunarMonthNames[leap - 1],
                        lunarDayName = lunarDayNames[temp],
                        zodiac = zodiacArr[(lunarYear - 4) % 12]
                    )
                }
                temp -= monthDays
                isLeap = true
                continue
            }

            val actualMonth = if (isLeap && i > leap + 1) i - 1 else i
            if (actualMonth > 12) break
            val monthDays = lunarMonthDays(lunarYear, actualMonth)

            if (temp < monthDays) {
                return LunarDate(
                    lunarYear = lunarYear,
                    lunarMonth = actualMonth,
                    lunarDay = temp + 1,
                    isLeapMonth = false,
                    ganZhiYear = getGanZhiYear(lunarYear),
                    lunarMonthName = lunarMonthNames[actualMonth - 1],
                    lunarDayName = lunarDayNames[temp],
                    zodiac = zodiacArr[(lunarYear - 4) % 12]
                )
            }
            temp -= monthDays
        }

        return LunarDate(
            lunarYear = lunarYear,
            lunarMonth = 1,
            lunarDay = 1,
            isLeapMonth = false,
            ganZhiYear = getGanZhiYear(lunarYear),
            lunarMonthName = "正",
            lunarDayName = "初一",
            zodiac = zodiacArr[(lunarYear - 4) % 12]
        )
    }

    private fun getGanZhiYear(lunarYear: Int): String {
        val ganIndex = (lunarYear - 4) % 10
        val zhiIndex = (lunarYear - 4) % 12
        return tianGan[ganIndex] + diZhi[zhiIndex]
    }
}
